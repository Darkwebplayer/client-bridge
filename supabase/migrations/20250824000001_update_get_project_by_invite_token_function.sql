...
--
-- Updated to check if the client is allowed to join the project
...
begin
  ...
  select
    ...
    (NOT EXISTS (SELECT 1 FROM allowed_clients WHERE project_id = p.id) OR
     EXISTS (SELECT 1 FROM allowed_clients WHERE project_id = p.id AND email = current_setting('request.jwt.claims', true)::json->>'email')) as client_allowed
  from
    projects p
